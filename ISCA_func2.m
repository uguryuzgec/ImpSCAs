function [Leader_score,Leader_pos,Convergence_curve]= ISCA_func2(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
    ...
    while t<Max_iteration
        ...
        rastgele=randi([1,N]);
            for j=1:size(X,2) % in j-th dimension
                ...
                r3=0.5*rand;
               %2. orta sonuç
                X(i,j)= pop(i,j)+(r1.*0.5*(sin(r2)+cos(r2)).*abs(r3.*pop(rastgele,j)-pop(i,j)));
